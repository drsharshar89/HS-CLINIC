...
function ImplantFixture() {
  ...
  const threadGeometry = useMemo(() => {
    ...
    points.push(new THREE.Vector2(0.3, 1.05));
...
export function ImplantScene() {
  return (
    <motion.div
      ...
      className="mx-auto h-80 w-80 md:h-[420px] md:w-[420px]"
    >
      <Canvas camera={{ position: [0, 0.5, 4], fov: 35 }} gl={{ antialias: true, alpha: true }}>
        {/* Ambient fill */}
        <ambientLight intensity={0.3} />

        {/* Key light — warm from upper-left */}
        <directionalLight position={[3, 4, 2]} intensity={1.2} color="#FFFAF0" />

        {/* Rim light — catches screw thread edges for drama */}
        <directionalLight position={[-3, 1, -2]} intensity={2.0} color="#D4AF37" />

        {/* Bottom fill — subtle gold uplighting */}
        <pointLight position={[0, -3, 1]} intensity={0.5} color="#C5A55A" />

        {/* Back rim — edge separation against dark bg */}
        <directionalLight position={[0, 0, -4]} intensity={1.5} color="#FFFFFF" />
        ...
        <PyramidWireframe position={[1.8, -1.3, -2.5]} scale={0.6} opacity={0.1} />
